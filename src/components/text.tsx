...
export const EOLocaleText: FunctionalComponent<IEOLocaleTextProps> = ({ children, html, id, ...values }) => {
  const context = useContext(EOLocaleContext);
  const result = context.translator.translate(id, values);

  return html ? <span dangerouslySetInnerHTML={{ __html: result }} /> : <Fragment>{result}</Fragment>;
};
